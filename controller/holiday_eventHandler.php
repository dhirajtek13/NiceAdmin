...
if ($jsonObj->request_type == 'addEdit') {
    ...
    $end_date = strtotime($hol_end_date);
    $start_date = strtotime($hol_start_date);
    $datediff = $end_date - $start_date;

    $holidays = round($datediff / (60 * 60 * 24)) + 1;
    ...
    if (!empty($user_data) && empty($err)) {
        if (!empty($id)) {
            ...
            $sqlQ = "UPDATE holidays SET hol_name=?,hol_desc=?,hol_start_date=?,hol_end_date=?, holidays=?   WHERE id=?";
            $stmt = $conn->prepare($sqlQ);
            $stmt->bind_param("sssssi", $hol_name, $hol_desc, $hol_start_date, $hol_end_date, $holidays, $id);
            $update = $stmt->execute();
            ...
            $sqlQ = "INSERT INTO holidays (hol_name, hol_desc, hol_start_date, hol_end_date, holidays)
                VALUES (?,?,?,?,?)";
            $stmt = $conn->prepare($sqlQ);
            $stmt->bind_param("sssss", $hol_name, $hol_desc, $hol_start_date, $hol_end_date,$holidays);
            $insert = $stmt->execute();
