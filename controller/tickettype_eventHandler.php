...
if($jsonObj->request_type == 'addEdit'){ 
    ...
    if(!empty($user_data) && empty($err)){ 
        if(!empty($id)){ 
            ...
            $sqlQ = "UPDATE ticket_types SET type_name=?  WHERE id=?"; 
            ...
            if($update){ 
                ...
                echo json_encode(['error' => 'Ticket Type Update request failed!']); 
            } 
        }else{ 
                // Insert event data into the database 
                //TODO- this is unless as of now untill add-user insert is not moved here
                $sqlQ = "INSERT INTO ticket_types (type_name)
                VALUES (?)"; 
                $stmt = $conn->prepare($sqlQ); 
                ...
                if ($insert) { 
                    ...
                    $output = [ 
                        'status' => 1, 
                        'msg' => 'New Ticket Type added successfully!' 
                    ]; 
                    echo json_encode($output); 
                }else{ 
                    echo json_encode(['error' => 'Ticket Type Add request failed!']); 
                } 
