...
if($jsonObj->request_type == 'addEdit'){ 
    ...
    if(!empty($user_data) && empty($err)){ 
        if(!empty($id)){ 
            ...
            $sqlQ = "UPDATE c_status_types SET type_name=?  WHERE id=?"; 
            ...
            if($update){ 
                ...
                echo json_encode(['error' => 'Ticket Status Update request failed!']); 
            } 
        }else{ 
                // Insert event data into the database 
                //TODO- this is unless as of now untill add-user insert is not moved here
                $sqlQ = "INSERT INTO c_status_types (type_name)
                VALUES (?)"; 
                $stmt = $conn->prepare($sqlQ); 
                ...
                if ($insert) { 
                    ...
                    $output = [ 
                        'status' => 1, 
                        'msg' => 'New Ticket Status added successfully!' 
                    ]; 
                    echo json_encode($output); 
                }else{ 
                    echo json_encode(['error' => 'Ticket Status Add request failed!']); 
                } 
    ...
    $sql = "DELETE FROM c_status_types WHERE id=$id"; 
    ...
    if($delete){ 
        ...
        echo json_encode(['error' => 'Status Delete request failed!']); 
    } 
}
