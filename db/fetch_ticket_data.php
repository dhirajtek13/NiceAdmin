...
if($_GET['ticket'] && isset( $select_data['id'])) {
    ...
    if($select_data['type_id'] == 2) {//if story then list the activities
        ...
        $sql3 = "SELECT id,ticket_id  FROM tickets WHERE parent_id=$ticket_id";
        ...
        if(!empty($userData)) {
            ...
            foreach ($userData as $key => $value) {
                $ticket_input_html .= '<div class="form-check mt-2">
                        <input class="form-check-input" type="radio" name="activity_selected" value="'.$key.'" id="activity_'.$key.'">
